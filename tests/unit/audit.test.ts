...
describe("audit log", () => {
  it("appends structured JSONL records without losing arguments", async () => {
    ...
    const record = {
      ...
      capability: { service: "git" as const, action: "commit", canonical: "git.commit" },
    ...
    const lines = (await readFile(resolveAuditPath({ HOME: home }), "utf8")).trim().split("\n");
  ...
  it("rejects when the audit directory cannot be created", async () => {
    ...
    await expect(appendAuditRecord({} as never, { HOME: home })).rejects.toThrow();
  });
});
