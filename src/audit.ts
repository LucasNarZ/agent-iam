...
export function renderCommand(tool: string, args: string[]): string {
  return [tool, ...args].map((value) => /^[a-zA-Z0-9_./:@%+=,-]+$/.test(value) ? value : JSON.stringify(value)).join(" ");
}

export async function appendAuditRecord(record: AuditRecord, env: NodeJS.ProcessEnv = process.env): Promise<void> {
