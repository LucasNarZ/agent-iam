...
export async function runWithShims(command: string, args: string[], options: RunnerOptions = {}): Promise<RunResult> {
  const baseEnv = { ...process.env, ...options.env };
  const originalPath = baseEnv.PATH ?? "";
  const entrypoint = resolve(options.entrypoint ?? process.argv[1] ?? join(process.cwd(), "dist/index.js"));
  ...
  try {
    await Promise.all(["git", "gh"].map(async (tool) => {
      const path = join(directory, tool);
      await writeFile(path, wrapper(nodePath, entrypoint, tool), "utf8");
