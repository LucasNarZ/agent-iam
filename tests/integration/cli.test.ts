...
describe("built CLI", () => {
  it("inspects policy and runs allowed and denied commands", async () => {
    ...
    const env = { ...process.env, HOME: home, PATH: `${bin}:${process.env.PATH}`, AGENTIAM_TEST_OUTPUT: output };
    const inspected = await exec(process.execPath, [cli, "inspect"], { env });
    expect(inspected.stdout).toContain("Default: DENY");
    expect(inspected.stdout).toContain("git.commit");
    const allowed = await exec(process.execPath, [cli, "run", "--", "sh", "-c", "git commit -m cli-test"], { env });
    expect(allowed.stdout).toBe("");
    expect(await readFile(output, "utf8")).toBe("commit\n-m\ncli-test\n");
    await expect(exec(process.execPath, [cli, "run", "--", "sh", "-c", "gh pr merge 42"], { env })).rejects.toMatchObject({ code: 126 });
  });
});
